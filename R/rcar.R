...
#'It handle only categorical data space.
#'@import arules
...
#'confidence thresholds of (20%, 80%)
#'
#'
#'inspect(head(rcar.fit$Rules))
#'
#'The first 6 rules are displayed, from left to right are shown for each CAR: the condition (lhs), the consequent(rhs),
#'the support, the confidence, and the frequence of the records that apply to the CAR.
#'
#'
#'rcar.fit
#'
#'It shows from left to right the number of nonzero coefficients (Df), the percent (of null) deviance
#'explained (dev) and the value of λ (Lambda).
#'
...
rcar<-function(data, y.rank=1, s=0.3, c=0.7, maxl=6,lambd){
    ...
    model$call<-"rcar(data, Support, Confidence, max.length, lambda)"
    list(Rules=rules,Model=model)
}
